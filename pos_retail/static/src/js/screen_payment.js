    ...
    screens.PaymentScreenWidget.include({
        ...
        renderElement: function () {
            ...
            this.$('.paid_partial').click(function () { // partial payment
                var order = self.pos.get_order();
                // 29/10/2019: code before modifications
                /*order.partial_payment = true;
                self.pos.push_order(order);
                self.gui.show_screen('receipt');*/
                
                var selectedB = document.querySelectorAll('.invoice_journals > .paymentmethods > .highlight');
                if (selectedB.length == 0){
                    return this.pos.gui.show_popup('dialog', {
                        title: 'Warning', 
                        body: 'Por favor selecciona un tipo de factura antes de continuar'
                    })
                }
                if (order.is_to_invoice()) {
        ...
        validate_order: function (force_validation) {
            ...
            if (order.paymentlines.models.length == 0) {
                return this.pos.gui.show_popup('dialog', {
                    title: 'Error',
                    body: 'No tiene metodos de pagos seleccionados'
                })
            }
            var selectedB = document.querySelectorAll('.invoice_journals > .paymentmethods > .highlight');
            if (selectedB.length == 0){
